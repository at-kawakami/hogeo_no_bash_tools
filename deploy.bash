...
echo "$asg_name "|bash -x /home/ubuntu/tools/bin/slack_notify.bash  -m "Start to BlueGreen Delploy" -i ':elf:' -n BGDeployer
...
if [ $? -ne 0 ];then
  ...
  echo "$asg_name is not Standby"|bash -x /home/ubuntu/tools/bin/slack_notify.bash  -m "Skip to BlueGreen Delploy" -i ':elf: -n BGDeployer'
  rm $Lock_file
  exit 0
else
...
$AWS autoscaling update-auto-scaling-group --min-size 1 --max-size 1 --desired-capacity 1 --auto-scaling-group-name $asg_name
...
while [ ! `$AWS ec2 describe-instances --filters "Name=tag:aws:autoscaling:groupName,Values=${asg_name}" "Name=instance-state-name,Values=running" \
	| $JQ -r '.Reservations[].Instances[].Tags[].Value' | grep deployed` ]
do
...
check all instances are deployed properly."|bash -x /home/ubuntu/tools/bin/slack_notify.bash  -m "Success to BlueGreen Delploy" -i ':elf:' -n BGDeployer
